...
MAPS_PATH=/var/www/minecraftMapsCommunity
LOG_TDIR=/var/www/minecraftLogsCommunity
LOGS_DAYS=7

#   End of configuration


if netstat -tulpn | grep -q ':25585'
then
        ONLINE=1
else
        ONLINE=0
...
server_launch() {
  ...
  if [ $SERVERMOD -eq 1 ]
  then
    cd $MC_PATH; screen -m -d -S $SCREEN_NAME ./run.sh
  else
    cd $MC_PATH; screen -m -d -S $SCREEN_NAME ./run.sh
  fi
}
...
server_stop() {
  ...
  screen -S $SCREEN_NAME -p 0 -X stuff "`printf "stop\r"`"; sleep 30
}
